...
function RecentOrdersList({
  ...
  if (error != null) {
    return (
      <p className='text-sm text-destructive'>Unable to load recent orders.</p>
    )
  }
  if (!orders.length) {
    ...
    <>
      ...
      <Main>
        <div className='mb-2 flex items-center justify-between space-y-2'>
          ...
          >
            <RefreshCw
              className={`mr-2 h-4 w-4 ${isAnyLoading ? 'animate-spin' : ''}`}
            />
            Refresh
          </Button>
        </div>
        ...
        {hasAnyError && (
          <div className='mt-4 text-center text-sm text-muted-foreground'>
            Some data could not be retrieved. Ensure you are logged in with a
            valid tenant and that the WMS backend is reachable.
          </div>
        )}
      </Main>
    </>
